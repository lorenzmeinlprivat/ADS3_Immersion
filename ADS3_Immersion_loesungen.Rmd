...
```{r A-09, fig.height = 3, fig.width = 5}
...
```
Da die logarithmische Transformation des Schadenaufwands nur die Höhe aber nicht die Ordnung/Reihenfolge der Werte. Der relative Einfluss (also die Richtung des Einflusses) der Merkmalsausprägungen von Variablen auf die Zielvariable kann also immer noch beobachtet werden. 
...
* Target Encoding: Hierbei wird der kategoriellen Variable ein numerischer Wert in Abhängigkeit des Einflusses auf die Zielvariable zugewiesen. Dies sorgt teilweise zu besseren Modellqualitäten, allerdings ist die Gefahr groß bei nicht richtiger Anwendung zu overfitten.
* **Weitere Encodings und Einfluss auf Praktikabilität und Performance**
